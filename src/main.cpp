...
const char* wifiSSID = "Great Southern LAN";
const char* wifiPWD = "Nada1975";
#endif
...
#define NTPHOST "au.pool.ntp.org"
#ifndef AUTODST
#define AUTODST
#endif
#endif
...
//-----------------------------------------------
/* Europe */
TimeChangeRule tcr1 = {"tcr1", First, Sun, Oct, 11, 660};  // standard/winter time, valid from last sunday of october at 3am, UTC + 1 hour (+60 minutes) (negative value like -300 for utc -5h)
TimeChangeRule tcr2 = {"tcr2", First, Sun, Apr, 10, 600};
// daylight/summer time, valid from last sunday of march at 2am, UTC + 2 hours (+120 minutes)
//-----------------------------------------------
...
uint8_t upperLimitLDR = 250;  // everything above this value will cause max brightness (according to current level) to be used (if it's higher than this)
uint8_t lowerLimitLDR = 10;   // everything below this value will cause minBrightness to be used
uint8_t minBrightness = 10;   // anything below this avgLDR value will be ignored
...
float factorLDR = 5.0;                  // try 0.5 - 2.0, compensation value for avgLDR. Set dbgLDR true & define DEBUG and watch the serial monitor. Looking...
const bool dbgLDR = true;              // ...for values roughly in the range of 120-160 (medium room light), 40-80 (low light) and 0 - 20 in the dark
#ifdef NODEMCU
uint8_t pinLDR = A0;  // LDR connected to A0 (nodeMCU only offers this one)
#else
...
#define SKETCHNAME "ClockSketch v7.3"
#define CLOCKNAME "Retro 7 Segment Clock v3 - The Final One(s), 3 LEDs/segment"

/* Start button config/pins----------------------------------------------------------------------------- */
#ifdef NODEMCU
const uint8_t buttonA = 4;  // momentary push button, 1 pin to gnd, 1 pin to d7 / GPIO_13
const uint8_t buttonB = 5;  // momentary push button, 1 pin to gnd, 1 pin to d5 / GPIO_14
#else
...
uint16_t colorSpeed = 250;               // controls how fast colors change, smaller = faster (interval in ms at which color moves inside colorizeOutput();)
...
const uint8_t brightnessLevels[5]{15,60,120,180,250};
// 0 - 255, brightness Levels (min, med, max) - index (0-2) will be saved to eeprom
uint8_t brightness = brightnessLevels[2];  // default brightness if none saved to eeprom yet / first run
#ifdef FADING
...
#define LED_PIN 14                     // led data in connected to GPIO_12 (d6/nodeMCU)
#else
...
;
void paletteSwitcher2(CRGB c1, CRGB c2, CRGB c3);
...
void setup() {
    ...
    ArduinoOTA.setHostname("digital-clock");
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        Serial.println(WiFi.localIP());  // if status is connected...
...
void readLDR() {  // read LDR value 5 times and write average to avgLDR
    ...
    if (runCounter == 5) {
        ...
        if (avgLDR > maxBrightness)
            avgLDR = maxBrightness;
        if (avgLDR >= upperLimitLDR && avgLDR < maxBrightness)
            avgLDR = maxBrightness;
        ;                                                     // if avgLDR is above upperLimitLDR switch to max current brightness
...
void setupClock() {
...
#endif
#ifdef DEBUG
    Serial.print(F("setupClock(): now is "));
    Serial.println(now());
#endif
#endif
    uint8_t lastInput = 0;
    ...
    Serial.print(2000 + setupTime.Year);
    Serial.print(F("/"));
